...
#        self.conv13 = nn.Conv2d(ci, kernel_num, (kernel_size[2], embed_dim))
        ...
        self.fc1 = nn.Linear(len(kernel_size) * kernel_num, len(kernel_size) * kernel_num)
        self.fc2 = nn.Linear(len(kernel_size) * kernel_num, class_num)
...
#        x3 = self.conv_and_pool(x, self.conv13)  # (batch, kernel_num)
        ...
        x=self.fc1(x)
        # x = self.dropout(x)
        logit = F.log_softmax(self.fc2(x), dim=1)
        return logit
